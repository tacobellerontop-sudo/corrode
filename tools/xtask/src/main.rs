...
fn package_windows(root: &std::path::Path) -> Result<(), String> {
	...
	if let Some(makensis) = makensis {
		...
		println!(
			...
			installer_dir
				.join(format!("Corrode-{version}-setup.exe"))
				.display()
		);
	} else {
		println!(
...
fn package() -> Result<(), String> {
	if std::env::args().nth(2).is_some() {
		return Err("Use cargo xtask package".into());
	}
	let arguments = [
		"build",
		"--release",
	...
	println!(
		"Unsigned package executable: {} ({} bytes)",
		destination.display(),
	...
	let result = enter_workspace().and_then(|()| {
		match std::env::args().nth(1).as_deref().unwrap_or("check") {
			"check" => run(&["fmt", "--all", "--", "--check"])
				...
				.and_then(|_| run(&["check", "-p", "Corrode", "--no-default-features", "--locked"]))
				.and_then(|_| policy()),
			"policy" => policy(),
			"licenses" => licenses(),
			"fuzz" => fuzz(),
