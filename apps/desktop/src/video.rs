	...
	fn start(
		...
		if self.requests.is_none() {
			...
			std::thread::Builder::new()
				.name("corrode-attachment-video".into())
				.spawn(move || {
		...
		if requests.is_closed() {
			return Err("Video worker stopped; restart Corrode");
		}
...
mod tests {
	...
	#[test]
	#[ignore = "CORRODE_VIDEO_SAMPLE supplies an offline clip; opens muted local output"]
	fn local_video_keeps_up_with_realtime() {
		let path = std::env::var("CORRODE_VIDEO_SAMPLE").or_else(|_| std::env::var("SEREIN_VIDEO_SAMPLE")).expect("CORRODE_VIDEO_SAMPLE path");
