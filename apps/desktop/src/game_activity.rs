...
trait Applications: Clone + Send + Sync + 'static {
	...
	/// Ask Discord to proxy caller-supplied image URLs. Corrode never fetches them itself.
	fn external(
		&self,
		id: Id,
		urls: &[String],
...
fn list_path() -> Option<PathBuf> {
	dirs::data_local_dir().map(|root| root.join("Corrode").join("detectable.json"))
}
