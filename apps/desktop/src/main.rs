...
/// Sign-in header strip: doubles as the window drag region.
const SIGN_IN_HEADER_HEIGHT: f32 = 44.0;
...
fn main() -> eframe::Result {
	...
	let icon = include_bytes!("../../../packaging/windows/corrode.png").as_slice();
	let options = eframe::NativeOptions {
		viewport: {
			let builder = egui::ViewportBuilder::default()
				...
				.with_app_id("cz.viceverse.corrode");
			let builder = builder
		...
		wgpu_options: eframe::egui_wgpu::WgpuConfiguration {
			wgpu_setup: eframe::egui_wgpu::WgpuSetup::CreateNew(
				eframe::egui_wgpu::WgpuSetupCreateNew {
					...
					// not wired to, which fails outright.
	...
	fn new(sample: Option<(Duration, Duration)>) -> Self {
		Self {
			enabled: sample.is_some()
				|| std::env::var_os("CORRODE_FRAME_DIAGNOSTICS").or_else(|| std::env::var_os("SEREIN_FRAME_DIAGNOSTICS")).is_some_and(|v| v == "1"),
			started: None,
	...
	fn drop(&mut self) {
		if self.enabled && self.sample.is_none() {
			...
			let _ = writeln!(
				...
				"[Corrode frames] callbacks={} without_input={} cpu_us_buckets(1000,2000,4000,8000,16000,32000,64000,above)={:?} reflows(total,consecutive)={:?}",
				self.frames,
				self.inputless,
				self.buckets,
				self.reflows
...
impl Desktop {
	fn new(
		...
		messaging.build = ui::design::Build {
			...
			} else if option_env!("CORRODE_CHANNEL").or(option_env!("SEREIN_CHANNEL")) == Some("nightly") {
		...
		// Windows rounds decorated windows only; opt the borderless surface in too.
		platform::window_effects::round_corners(&window);
		// The GPU surface and window effects are configured at startup. Opaque launches
		// keep the same native path as builds without window effects.
		Ok(Self {
		...
		egui::CentralPanel::default()
			...
			.show(ui, |ui| {
				...
				egui::Frame::NONE
					...
					.show(ui, |ui| {
						ui.horizontal(|ui| {
							ui.label(ui::design::semibold(ui, "Corrode", 16.0).color(p.muted));
							ui.with_layout(
				...
				egui::Panel::top("sign-in-header")
					...
					.show(ui, |ui| {
						...
						ui.horizontal_centered(|ui| {
							...
							ui.add_space(8.0);
							ui.label(ui::design::semibold(ui, "Corrode", 16.0).color(p.text_strong));
							ui.add_space(8.0);
				...
				let button = ui
					.add_enabled_ui(ready, |ui| {
						if returning {
							...
							ui::design::primary_icon_button(ui, ui::icons::Icon::Corrode, label)
						}
					})
					.inner;
	...
	fn sign_in_disclosures(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
		let p = ui::design::palette(ui);
		if ui::design::disclosure(ui, "About Corrode", self.about_open).clicked() {
			...
			egui::Frame::NONE
				...
				.show(ui, |ui| {
					ui.add(
						egui::Label::new(
							egui::RichText::new(
								"For owners who already hold a valid Discord session token, for example from another signed-in Corrode install. Passwords and 2FA are never used here; this bypasses Discord's hosted login page entirely.",
							)
							.size(12.0)
							.color(p.muted),
						)
	...
	fn poll(&mut self, ctx: &egui::Context) {
				...
				match cache.receive.try_recv() {
					...
					Err(std::sync::mpsc::TryRecvError::Disconnected) => {
							...
							|| self.messaging.channel_preferences_load_pending
						{
							self.messaging.channel_preferences_reload = false;
							self.messaging.channel_preferences_load_pending = false;
							self.messaging.channel_preferences_status = "Local storage worker stopped; restart Corrode to restore channel preferences.";
						}
			...
			match outcome {
				...
				credentials::Outcome::Forgotten(result) => {
					...
					self.credential_status = if result.is_ok() {
						...
						"Could not remove saved login; remove cz.viceverse.corrode / discord-session in your OS credential manager"
					};
				}
			}
		}
		...
		for mut event in events {
			...
			if ready && self.state.auth == AuthState::Authenticated {
				...
				if let Some(store) = &self.store {
					...
					// Writing an existing entry is an access-controlled credential-store operation,
					// so only write when the roster says none exists or the token is new.
		...
		if let Some(failure) = terminal {
			for (setting, legacy, scope) in [
				(
					"CORRODE_MEMBER_DIAGNOSTICS",
					"SEREIN_MEMBER_DIAGNOSTICS",
					"members",
				),
				(
					"CORRODE_GATEWAY_DIAGNOSTICS",
					"SEREIN_GATEWAY_DIAGNOSTICS",
					"gateway",
				),
			] {
				let enabled = [setting, legacy].iter().any(|key| {
					std::env::var_os(key).as_deref() == Some(std::ffi::OsStr::new("1"))
				});
				if enabled {
					use std::io::Write;
...
impl eframe::App for Desktop {
	...
	fn logic(&mut self, ctx: &egui::Context, _: &mut eframe::Frame) {
		...
		if let Some(tray) = &mut self.tray {
			// The tray worker can refill event bits while this frame consumes them.
			for _ in 0..4 {
		...
		// Occlusion can occur during fullscreen transitions; it is not a request
