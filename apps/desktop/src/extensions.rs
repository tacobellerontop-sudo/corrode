...
// The upstream extension catalog was removed with the release feed: only
// bundled themes and local files are available.
...
pub(crate) fn starters() -> Result<Vec<Starter>, String> {
	let packages: &[(&'static [u8], &'static str)] = &[
		...
		(
			include_bytes!(
				"../../../examples/extensions/packages/message-delete-protector.corrode-extension"
			),
...
pub fn demo_check_examples() -> Result<bool, String> {
	...
	let root = std::env::temp_dir().join(format!("corrode-theme-check-{}", std::process::id()));
...
fn run(root: &Path, job: Job, gate: &Gate) -> Result<Event, String> {
	...
	match job {
		...
		Job::RefreshCatalog { demo } => {
			...
			Err(
				"The extension catalog is not configured in this fork. Install extensions from local files."
					.into(),
			)
		}
...
// No remote catalog remains to cache; covered by unit tests only.
#[allow(dead_code)]
...
fn demo_preview(id: &str) -> Option<&'static [u8]> {
	...
	{
		match id {
			"corrode-ocean" => Some(include_bytes!("../../../extensions/previews/ocean.png")),
			_ => None,
		}
	}
...
mod tests {
	...
	impl Profile {
		fn new() -> Self {
			...
			let path =
				std::env::temp_dir().join(format!("corrode-extension-test-{}", digest(&nonce)));
	...
	fn bundled_themes_open_without_installing_and_preserve_preview_intent() {
		...
		let original =
			extensions::parse_package(include_bytes!("../../../extensions/ocean.corrode-extension"))
				.unwrap();
	...
	fn message_event_worker_requires_grants_serializes_storage_and_rejects_cancelled_jobs() {
		...
		let package = extensions::parse_package(include_bytes!(
			"../../../examples/extensions/packages/message-counter.corrode-extension"
		))
		.unwrap();
		enable(
			&root,
