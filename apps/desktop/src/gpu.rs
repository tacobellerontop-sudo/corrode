...
//! but the display is usually wired to the discrete card. Rank the adapters that can actually
...
pub fn select(
	...
	let Some(adapter) = usable.first() else {
		...
		return Err(if available.is_empty() {
			"no GPU adapter found; install a DirectX, Vulkan or OpenGL driver".to_owned()
		} else {
			format!("no GPU adapter can draw this window; found {available}")
		});
	};
	eprintln!("[Corrode] GPU adapter: {}", describe(&adapter.get_info()));
	Ok((*adapter).clone())
}
