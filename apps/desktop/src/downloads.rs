		...
		let spawn = std::thread::Builder::new()
			...
			.spawn(move || {
				...
				if copied_ok && let Some(owner) = owner {
					// Video paste needs its one bounded file until the next copy or
...
impl CopyFile {
	fn create(filename: &str, cleanup_failed: Arc<AtomicBool>) -> Result<Self, &'static str> {
		...
		let root = std::env::temp_dir().join(format!("corrode-clipboard-{suffix}"));
...
impl<'a> Partial<'a> {
	fn create(destination: &Path, cleanup_failed: &'a AtomicBool) -> Result<Self, &'static str> {
		...
		let path = parent.join(format!(".Corrode-{random}.partial"));
...
mod tests {
	...
	async fn explicit_download_stream_limits_cancel_and_atomic_replacement() {
		...
		let root =
			std::env::temp_dir().join(format!("corrode-download-tests-{}", std::process::id()));
