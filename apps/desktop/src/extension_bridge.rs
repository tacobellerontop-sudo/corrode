...
impl Bridge {
	...
	pub fn tick(
		...
		if self.host.is_none() {
			let root = if demo {
				Some(std::env::temp_dir().join("corrode-extension-demo"))
			} else {
				dirs::data_local_dir().map(|root| root.join("Corrode").join("extensions"))
			};
			...
			match request {
				...
				ExtensionRequest::ExportTheme { package } if self.theme_picker.is_none() => {
					...
					let future = platform::save::theme_destination(
						window.clone(),
						&format!("{}.corrode-extension", package.manifest.id),
					);
					let ctx = ctx.clone();
					runtime.spawn(async move {
...
mod tests {
	...
	fn cancelled_import_cannot_replace_the_catalog_bytes_the_user_approved() {
		...
		let imported = InstallSource::Local {
			path: PathBuf::from("original.corrode-extension"),
			sha256: "a".repeat(64),
			manifest: manifest.clone(),
		};
