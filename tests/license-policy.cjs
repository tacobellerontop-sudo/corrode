...
const fixtureRoot = fs.mkdtempSync(path.join(tempParent, 'corrode-license-policy-'));
...
${license === null ? '' : `license = "${license}"\n`}`);
  ...
  assert.ok(path.basename(resolved).startsWith('corrode-license-policy-'), 'Cleanup must target this fixture directory');
