...
const source = fs.readFileSync('crates/platform/src/login-handoff.js', 'utf8').replace('__CORRODE_LOGIN_CAPABILITY__', 'SYNTHETIC_CAPABILITY:');
...
const linuxBridge = fs.readFileSync('crates/platform/src/login-linux-bridge.js', 'utf8')
  .replace('__CORRODE_LOGIN_CAPABILITY__', linuxCapability);
...
function linuxSetup({ origin = 'https://discord.com', frame = false } = {}) {
  ...
  const context = {
    ...
    webkit: { messageHandlers: { CORRODELogin: { postMessage: value => messages.push(value) } } },
  ...
  return { context, messages, expire: () => now = 600001,
    take: () => context['__corrode_login_take_' + linuxCapability.slice(0, -1)]() };
}
{
  ...
  assert.equal(Object.getOwnPropertyDescriptor(context, '__corrode_login_take_' + linuxCapability.slice(0, -1)).writable, false);
