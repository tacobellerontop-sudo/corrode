...
/// `CORRODE_VOICE_DIAGNOSTICS=1` the bounded raw value also reaches stderr for reports.
fn deletion_reason(raw: Option<&str>) -> Option<&'static str> {
	let raw = raw?;
	if raw.len() > 64 {
		return Some("Discord ended the stream");
	}
	if std::env::var_os("CORRODE_VOICE_DIAGNOSTICS").or_else(|| std::env::var_os("SEREIN_VOICE_DIAGNOSTICS")).is_some_and(|v| v == "1") {
		eprintln!("[Corrode voice Stream] discord_delete_reason={raw}");
	}
	Some(match raw {
