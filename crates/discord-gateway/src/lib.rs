...
impl Diagnostics {
	...
	fn record_to(&mut self, label: &'static str, writer: &mut impl std::io::Write) {
		...
		self.remaining -= 1;
		self.bytes -= bytes;
		let _ = writeln!(writer, "[Corrode {}] {label}", self.scope);
	}
}
...
async fn run_inner(
	...
	let mut member_diagnostics = Diagnostics::new(
		"members",
		std::env::var_os("CORRODE_MEMBER_DIAGNOSTICS").or_else(|| std::env::var_os("SEREIN_MEMBER_DIAGNOSTICS")).as_deref() == Some(std::ffi::OsStr::new("1")),
	);
	let mut gateway_diagnostics = Diagnostics::new(
		"gateway",
		std::env::var_os("CORRODE_GATEWAY_DIAGNOSTICS").or_else(|| std::env::var_os("SEREIN_GATEWAY_DIAGNOSTICS")).as_deref()
...
mod member_tests {
	...
	fn diagnostics_are_opt_in_byte_bounded_redacted_and_tolerate_closed_output() {
		...
		let line = "[Corrode gateway] unsupported dispatch ignored\n";
		...
		assert_eq!(output, "[Corrode members] \u{e9}\n".as_bytes());
