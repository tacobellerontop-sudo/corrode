...
pub(crate) enum Scope {
	...
	StreamReceive,
	ScreenAudio,
	/// Screen capture: `receive` counts pictures taken from the pipeline, `encode`
...
/// Application-audio capture counters, reported under `Scope::ScreenAudio`.
#[derive(Clone, Copy)]
#[allow(dead_code)]
...
struct Report {
	...
	/// Application-audio capture: inputs allowed, captures started, applications
...
impl Metrics {
	pub fn new(scope: Scope) -> Self {
		...
		let send = REPORTER.get_or_init(|| {
			if std::env::var_os("CORRODE_VOICE_DIAGNOSTICS").or_else(|| std::env::var_os("SEREIN_VOICE_DIAGNOSTICS")).is_none_or(|v| v != "1") {
				return None;
			}
...
fn write_report(report: Report, bytes: &mut usize, writer: &mut impl Write) -> bool {
	let mut line = format!(
		"[Corrode voice {:?}] debug={} at_ms={} window_ms={} wakes={} resets={} drops={} stalls={} noise_frames={} stages(calls,total_us,max_us): echo_render={:?} echo_capture={:?} noise={:?} encode={:?} mix={:?} receive={:?}",
		report.scope,
