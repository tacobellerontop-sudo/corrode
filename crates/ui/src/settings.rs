...
impl Page {
	...
	fn description(self) -> &'static str {
		match self {
			...
			Self::Keybinds => "Keyboard shortcuts for Corrode.",
			Self::Storage => "What Corrode keeps on this device.",
			Self::Updates => "Keep Corrode up to date on this device.",
...
impl MessagingUi {
	...
	fn settings_navigation(&mut self, ui: &mut egui::Ui, state: &State) {
		...
		egui::ScrollArea::vertical()
			...
			.show(ui, |ui| {
				...
				ui.add_space(12.0);
				ui.label(
					RichText::new(format!("Corrode {}", self.build.version))
						.size(12.0)
						.color(colors.muted),
				);
				ui.label(
	...
	fn general_settings(&mut self, ui: &mut egui::Ui, _demo: bool) {
		design::group(ui, "Startup", |ui| {
			ui.add_enabled_ui(self.startup_available && !self.startup_busy, |ui| {
				design::switch(
					ui,
					"Open Corrode when your computer starts",
					Some("Corrode signs in and connects in the background."),
			...
			if !self.startup_available {
				design::hint(ui, "Automatic startup is available on Windows.");
		...
		design::group(ui, "Window", |ui| {
			...
			ui.add_enabled_ui(self.tray_available, |ui| {
				design::switch(
					ui,
					"Keep Corrode in the system tray",
					Some("Closing the window keeps Corrode in the notification area. Quit from its menu to exit."),
		...
		design::group(ui, "Graphics", |ui| {
			let detail = if self.gpu_adapter.is_empty() {
				"Takes effect the next time Corrode starts.".to_owned()
			} else {
				format!(
					"Currently drawing with {}. Takes effect the next time Corrode starts.",
					self.gpu_adapter
				)
			};
	...
	fn appearance_settings(&mut self, ui: &mut egui::Ui, demo: bool) {
		...
		design::group(ui, "Window effects", |ui| {
			design::switch(
				...
				"Transparency & blur",
				Some(
					"Restart Corrode after changing this. Themes can customize effects while enabled.",
				),
	...
	fn storage_page(&mut self, ui: &mut egui::Ui, state: &State) {
		design::group(ui, "Local storage", |ui| {
			...
			design::hint(
				ui,
				"Messages and drafts are cached on this device inside bounded, account-isolated files. Cache data is not encrypted by Corrode; saved login tokens use the OS credential store.",
			);
		});
		design::group(ui, "Your privacy", |ui| {
			design::hint(
				ui,
				"Corrode does not collect telemetry or upload diagnostics. Discord retains service-side data according to its own policies.",
			);
		});
	}
}
