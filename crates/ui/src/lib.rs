...
fn snapshot_member_rows<'a>(
	state: &'a State,
	list: &'a model::MemberList,
	own: model::PresenceStatus,
	...
	let mut members: Vec<_> = list
		...
		.filter_map(|slot| {
			...
			let status = profiles::live_self_status(member.user.id, state, own)
				.or_else(|| {
					profiles::member_presence(state, member, list.guild)
						.0
				});
			let offline =
				!status.is_some_and(|status| matches!(status, "online" | "idle" | "dnd"));
			let role = list
				.guild
...
impl MessagingUi {
		...
		state.invites.insert(
			...
			(
				...
				Some(Ok(model::InvitePreview {
					guild: Id(424242),
					embed: model::Embed {
						title: Some("Corrode community".into()),
						..Default::default()
					},
				})),
			),
	...
	fn member_rows(&mut self, ui: &mut egui::Ui, state: &mut State, commands: &mut Vec<Command>) {
		...
		// Thread and DM snapshots contain people only; paged channel lists supply their own headers.
		let snapshot_rows = (!list.lazy
			&& state
				.channel(list.channel)
				.is_some_and(|channel| matches!(channel.kind, 1 | 3 | 10..=12)))
		.then(|| snapshot_member_rows(state, list, self.own_presence.status));
		...
		let row_count = if lazy {
			...
			snapshot_rows.as_ref().map_or(list.slots.len(), Vec::len)
		};
		...
		let output = self
			...
			.show_rows(ui, 42.0, row_count, |ui, range| {
				...
				for index in range {
					...
					} else if let Some(rows) = &snapshot_rows {
					...
					match slot {
						Some(model::MemberSlot::Group(id)) => {
							...
							let text = list
								...
								.map(|(_, count)| *count)
								.or_else(|| snapshot_rows.as_ref().and_then(|rows| rows[index].1))
						...
						let status = profiles::live_self_status(
							member.user.id,
							state,
						...
						.or(status);
							...
							inner.push_id(member.user.id.0, |ui| {
								...
								if !online {
									// Grey offline pfps: wash the avatar toward the sidebar so
									// offline rows read as inactive next to the muted name.
									ui.painter().circle_filled(
										avatar.rect.center(),
										avatar.rect.width() / 2.0,
										colors.sidebar.gamma_multiply(0.55),
									);
								}
				...
				if self.guild.is_none() {
					...
					ui.horizontal(|ui| {
						...
						let find = ui
							...
							.inner
							.on_hover_text("Search loaded conversations (Ctrl+K)");
						find.widget_info(|| {
							egui::WidgetInfo::labeled(
								egui::Role::Button,
								ui.is_enabled(),
	...
	fn composer(
		...
		// Some IMEs repeatedly emit empty preedits while idle. Only real
			...
			ctx.input_mut(|input| {
				...
				if has_paste || shortcut {
					...
					input.events.retain_mut(|event| match event {
						...
						// Alt+V can also produce a platform text character.
                ...
                ui.horizontal(|ui| {
                    ...
                    let attach = if !editing_here && self.slash_commands.composer_badge(ui, state, &mut self.avatars) {
                        ...
                        Some(ui
                            ...
                            .inner
                            .on_hover_text("Choose, drop, or paste files (Ctrl/Alt+V). Up to 10 files and 500 MB total; account limits may be lower. Send starts the upload."))
                    };
...
mod composer_tests {
	...
	#[test]
	fn composer_enter_ignores_idle_ime_updates() {
	...
	#[test]
	fn dm_snapshots_separate_online_from_offline() {
		fn collect_text(shape: &egui::Shape, text: &mut Vec<String>) {
			match shape {
				egui::Shape::Text(shape) => text.push(shape.galley.job.text.clone()),
				egui::Shape::Vec(shapes) => {
		...
		let users: Vec<_> = (1..=3).map(user).collect();
		let mut state = State {
			demo: true,
			gateway_connected: true,
			selected: Some(Id(7)),
			...
			direct_presences: vec![
				model::MemberPresence {
					user: Id(1),
					// Stale snapshot: your own rows track the live local status instead.
		...
		let online = text
			.iter()
			.position(|label| label == "Online - 1")
			.expect("online section");
		let offline = text
			.iter()
			.position(|label| label == "Offline - 2")
			.expect("offline section");
		assert!(
			online < offline,
			"online members must render before offline members"
		);
		for id in 1..=3 {
			assert!(text.iter().any(|label| label == &format!("DM {id}")));
		}
		output.drop_without_applying_deltas();
	}
