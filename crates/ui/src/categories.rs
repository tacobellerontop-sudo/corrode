...
fn eyebrow_row(
	...
	ui.allocate_ui_with_layout(
		...
		|ui| {
			ui.add_space(8.0);
			ui.add(egui::Label::new(design::eyebrow(ui, label, color)).selectable(false));
		},
	)
	.response
	.rect
...
impl MessagingUi {
	/// Right-click menu on the pinned-DM section heading: rename and recolor it.
	/// Writes go straight into the persisted channel preferences.
	fn pinned_section_menu(&mut self, ui: &mut egui::Ui, rect: &egui::Rect) {
		let response = ui.interact(
			*rect,
			ui.scope_id().with("pinned-heading"),
			egui::Sense::click(),
		);
		// The menu holds editing controls, so only outside clicks may dismiss it;
		// the default would close on every click inside, including the text edit.
		egui::Popup::context_menu(&response)
			...
			let mut name =
				std::mem::take(&mut self.channel_preferences.pinned_name);
			let name_changed = ui
				.add(
					egui::TextEdit::singleline(&mut name)
						.hint_text(model::ChannelPreferences::DEFAULT_PINNED_NAME),
				)
				.changed();
			let mut store_name = false;
			if name_changed {
				store_name = self.channel_preferences.set_pinned_name(&name);
				// Keep the sanitized value so the box never shows unpersistable text.
				name = std::mem::take(&mut self.channel_preferences.pinned_name);
			}
			self.channel_preferences.pinned_name = name;
			if store_name {
				self.channel_preferences_changed = true;
			}
			ui.add_space(4.0);
			ui.label("Color");
			// Inline full-spectrum picker: no nested popup, so menu
			// interactions stay reliable while dragging colors.
			let fallback = design::palette(ui).muted;
			let current = self
				.channel_preferences
				.pinned_color_rgb()
				.map_or(fallback, |[r, g, b]| egui::Color32::from_rgb(r, g, b));
			...
			if ui.small_button("Reset").clicked() {
				let name_reset = self.channel_preferences.set_pinned_name("");
				let color_reset = self.channel_preferences.set_pinned_color(None);
				if name_reset || color_reset {
					self.channel_preferences_changed = true;
				}
				ui.close();
			}
			ui.small("Section name and color are saved on this device.");
		});
	}
				...
				for index in range {
					...
					match row {
						CachedRow::Heading(heading) => {
							let (label, color) = if heading == Heading::Pinned {
								(
									self.channel_preferences.pinned_heading().to_owned(),
									self.channel_preferences
										.pinned_color_rgb()
										.map_or(colors.muted, |[r, g, b]| {
											egui::Color32::from_rgb(r, g, b)
										}),
								)
							} else {
								(heading.label().to_owned(), colors.muted)
							};
							let rect = eyebrow_row(ui, &label, color, row_height);
							if heading == Heading::Pinned {
								self.pinned_section_menu(ui, &rect);
							}
						...
						CachedRow::Channel(channel, slot, nested) => {
							...
							// Kinds Corrode cannot render keep Discord's own destination.
...
mod tests {
	...
	#[test]
	fn pinned_dms_lift_above_the_dm_list() {
		let ctx = egui::Context::default();
		let mut state = test_support::demo_state();
		state.channels = (1..=3)
			.map(|id| {
				let mut c = channel(id, 3, id as i32, None);
				c.guild = None;
				c.name = format!("DM {id}");
				c
			})
		...
		let before = render(&mut view, &mut state);
		assert!(
			!before.iter().any(|s| s == "PINNED"),
			"no pinned section before pinning"
		);
		let available = view.shortcuts_available(&state);
		let intent =
			ShortcutView::new(&view.channel_preferences, available)
				.toggle(Shortcut::Pinned, Id(3));
		view.apply_shortcut(&state, intent);
		let after = render(&mut view, &mut state);
		let pinned = after
			.iter()
			.position(|s| s == "PINNED")
		...
		let other = after
			...
			.expect("unpinned DM");
		assert!(
			pinned < lifted && lifted < other,
			"pinned DM lifts above the DM list"
		);
		let _ = view.channel_preferences.set_pinned_name("Starred");
		let renamed = render(&mut view, &mut state);
		assert!(
			renamed.iter().any(|s| s == "STARRED"),
			"custom section name renders"
		);
		assert!(
			!renamed.iter().any(|s| s == "PINNED"),
			"default heading is replaced"
		);
	}
	#[test]
	fn pinned_section_menu_stays_open_while_editing() {
		let ctx = egui::Context::default();
		let mut state = test_support::demo_state();
		state.channels = (1..=2)
			.map(|id| {
			...
			.collect();
		let mut view = MessagingUi::default();
		let render = |view: &mut MessagingUi,
		              state: &mut State,
		...
		let shown = render(&mut view, &mut state, vec![]);
		let heading = shown
			.iter()
			.find(|(s, _)| s == "PINNED")
			.expect("pinned section")
		...
		render(
			...
			&mut state,
			vec![
				egui::Event::PointerMoved(heading),
				click(heading, egui::PointerButton::Secondary, true),
			],
		);
		render(
			&mut view,
			&mut state,
			vec![click(heading, egui::PointerButton::Secondary, false)],
		);
		let open = render(&mut view, &mut state, vec![]);
		let editor = open
			.iter()
			.find(|(s, _)| s == "Section name")
			.expect("rename menu opened")
			.1
			.center();
		render(
			&mut view,
			...
			vec![
				...
				click(editor, egui::PointerButton::Primary, true),
			],
		);
		render(
			&mut view,
			&mut state,
			vec![click(editor, egui::PointerButton::Primary, false)],
		);
		let stayed = render(&mut view, &mut state, vec![]);
		assert!(
			stayed.iter().any(|(s, _)| s == "Section name"),
			"clicking inside the rename menu must not dismiss it"
		);
		// Typing in the menu renames the section live.
		let edit = stayed
			.iter()
			.find(|(s, _)| s == "Pinned")
			.expect("name box hint")
			.1
			.center();
		render(
			&mut view,
			...
			vec![
				...
				click(edit, egui::PointerButton::Primary, true),
			],
		);
		render(
			&mut view,
			&mut state,
			vec![click(edit, egui::PointerButton::Primary, false)],
		);
		render(
			&mut view,
			&mut state,
			vec![egui::Event::Text("Star".into())],
		);
		let renamed = render(&mut view, &mut state, vec![]);
		assert_eq!(view.channel_preferences.pinned_name, "Star");
		assert!(
			renamed.iter().any(|(s, _)| s == "STAR"),
			"typed section name renders"
		);
	}
	#[test]
	fn pinned_section_color_renders_on_the_heading() {
		let ctx = egui::Context::default();
		let mut state = test_support::demo_state();
		state.channels = (1..=2)
			.map(|id| {
		...
		let _ = view
			.channel_preferences
			.set_pinned_color(Some([0x12, 0x34, 0x56]));
		let mut found = false;
		let mut output = ctx.run_ui(
			egui::RawInput {
				screen_rect: Some(egui::Rect::from_min_size(
			...
			|ui| view.sidebar(ui, &mut state, "Direct Messages", &mut vec![]),
		);
		output.textures_delta.clear();
		for shape in &output.shapes {
			let mut stack = vec![&shape.shape];
			while let Some(shape) = stack.pop() {
				match shape {
					egui::Shape::Text(t) if t.galley.job.text == "PINNED" => {
						found = true;
						assert!(
							t.galley
								.job
								.sections
								.iter()
								.any(|s| s.format.color == egui::Color32::from_rgb(0x12, 0x34, 0x56)),
							"stored section color must reach the heading"
						);
					}
		...
		assert!(found, "pinned section renders");
		output.drop_without_applying_deltas();
		// Reset through the menu clears the override like a real click would.
		let render_labels =
			|view: &mut MessagingUi, state: &mut State, events: Vec<egui::Event>| {
				let mut output = ctx.run_ui(
					egui::RawInput {
						screen_rect: Some(egui::Rect::from_min_size(
							egui::Pos2::ZERO,
		...
		let click =
			|pos: egui::Pos2, button: egui::PointerButton, pressed: bool| {
				egui::Event::PointerButton {
					pos,
					button,
					pressed,
		...
		let shown = render_labels(&mut view, &mut state, vec![]);
		let heading = shown
			.iter()
			.find(|(s, _)| s == "PINNED")
			.expect("pinned heading")
			.1
			.center();
		render_labels(
			&mut view,
		...
		let open = render_labels(&mut view, &mut state, vec![]);
		let reset = open
			.iter()
			.find(|(s, _)| s == "Reset")
			.expect("reset row")
		...
		render_labels(
			...
			vec![
				...
				click(reset, egui::PointerButton::Primary, true),
			],
		);
		render_labels(
			&mut view,
			&mut state,
			vec![click(reset, egui::PointerButton::Primary, false)],
		);
		let _ = render_labels(&mut view, &mut state, vec![]);
		assert_eq!(
			view.channel_preferences.pinned_color_rgb(),
			None,
			"reset clears the color override"
		);
	}
