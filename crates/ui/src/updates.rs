...
impl MessagingUi {
	/// Whether the account card grows an update row, which it does only while Corrode's own
		...
		design::card(ui, |ui| {
			ui.horizontal(|ui| {
				...
				ui.vertical(|ui| {
					...
					ui.label(
						design::semibold(ui, format!("Corrode {}", self.build.version), 17.0)
		...
		design::group(ui, "Preferences", |ui| {
			ui.add_enabled_ui(self.updates.supported || demo, |ui| {
				design::switch(
					ui,
					"Auto update",
					Some("Download updates in the background. Restart when you are ready. Corrode still checks at startup and periodically when this is off."),
...
mod tests {
	...
	fn hidden_title_bar_moves_the_update_prompt_above_the_account_card() {
		...
		// With Corrode's own title bar the prompt stays up there, not in the sidebar.
