//! Corrode theme tokens, presets and typography shared by every native view.
//!
//! The palette is resolved from egui's light/dark mode plus a process-wide [`Variant`]
//! (the cool Corrode neutrals, deep black, blue-grey, or a gradient recolour). Gradient
...
/// Corrode blurple: the house accent, packed for call sites that speak in integer colours.
...
fn dark_common(
	...
	Palette {
		...
		selected,
		border,
		text_strong: rgb(0xdbdee1),
		// Discord secondary lifted a shade to hold the 7.0 body-text contrast bar.
		text: rgb(0xbec3cc),
		muted: rgb(0x949ba4),
		link: rgb(0x00a8fc),
		accent: PRIMARY,
...
const WEIGHTS_KEY: &str = "corrode-font-weights";
...
/// Space reserved at the left of window strips for traffic lights; Windows draws none.
pub const TRAFFIC_LIGHT_INSET: f32 = 0.0;
...
/// Deterministic fallback avatar colours drawn from the Corrode palette, keyed by the display name.
...
mod sign_in_widget_tests {
	...
	fn account_row_and_disclosure_click_and_label_themselves() {
		for light in [false, true] {
				...
				let output = ctx.run_ui(
					...
					|ui| {
						ui.scope_builder(egui::UiBuilder::new().max_rect(area), |ui| {
							...
							clicks.1 = disclosure(ui, "About Corrode", false).clicked();
						});
					},
				);
			...
			let (text, _) = run(vec![]);
			for expected in ["Riley Quinn", "@riley", "About Corrode", "RQ"] {
