...
/// Live local status for your own rows: list snapshots can lag or omit your
/// presence while the account card is already current.
pub(crate) fn live_self_status(
	user: Id,
	state: &State,
...
pub(crate) fn profile_opener_id() -> egui::Id {
	egui::Id::unique("corrode-profile-opener")
}
...
pub fn show(
	...
	confirm_links: bool,
	anchor: Pos2,
	own: Option<model::PresenceStatus>,
) -> Option<Action> {
	let colors = design::palette(ui);
	...
	let status = own
		.and_then(|own| live_self_status(user.id, state, own))
		.or(status);
