...
fn demo_embeds(id: u64) -> Vec<Embed> {
	...
	vec![Embed {
        ...
        color: Some(0x68ada4),
        author: Some(EmbedAuthor { name: "Corrode · synthetic example".into(), ..Default::default() }),
...
pub fn code_demo_state() -> State {
	...
	let texts = [
		...
		"```json\n{\"name\": \"Corrode\", \"version\": 1, \"voice\": true, \"tags\": [\"native\", null]}\n```",
...
pub fn seed_stickers(state: &mut State) {
	...
	state.stickers.packs = vec![model::StickerPack {
		id: Id(9200),
		name: "Corrode Friends (synthetic)".into(),
		stickers: vec![
