...
function page({invisible = false, origin = "https://corrode-captcha.verification.invalid", subframe = false, href = origin + "/"} = {}) {
  ...
  vm.runInNewContext(template.replace("__CORRODE_CAPTCHA_CONFIG__", JSON.stringify(config)), {window, document, location: {origin, href}});
  listeners.DOMContentLoaded?.();
  window.CORRODECaptchaLoaded?.();
...
assert.equal(current.calls[0][1].src, "https://js.hcaptcha.com/1/api.js?onload=CORRODECaptchaLoaded&render=explicit&recaptchacompat=off&host=service-sitekey.react-native.hcaptcha.com");
...
assert.ok(page({origin: "null", href: "corrode-captcha://verification.invalid/"}).params);
assert.equal(page({href: "corrode-captcha://verification.invalid/other"}).calls.length, 0);
