...
mod native {
		...
		pub fn new(
			...
			let previous = unsafe {
				if GetWindowThreadProcessId(hwnd, None) != GetCurrentThreadId()
					|| !GetPropW(hwnd, w!("Corrode.TrayState")).is_invalid()
				{
					return Err(UNAVAILABLE);
				}
			...
			let (notification, restart) = unsafe {
				(
					RegisterWindowMessageW(w!("Corrode.TrayCallback")),
			...
			for (slot, unit) in data.szTip.iter_mut().zip("Corrode".encode_utf16()) {
				*slot = unit;
			}
			let tray = Self {
				state: Rc::new(State {
			...
			unsafe {
				AppendMenuW(menu, MF_STRING, SHOW, w!("Show Corrode")).map_err(|_| UNAVAILABLE)?;
				...
				if SetWindowLongPtrW(hwnd, GWLP_WNDPROC, callback as *const () as isize) == 0 {
					let _ = RemovePropW(hwnd, w!("Corrode.TrayState"));
					drop(Rc::from_raw(reference));
					return Err(UNAVAILABLE);
				}
			}
	...
	unsafe extern "system" fn callback(
		...
		let pointer = unsafe { GetPropW(hwnd, w!("Corrode.TrayState")).0.cast::<State>() };
		if pointer.is_null() {
		...
		if message == WM_NCDESTROY {
			...
			let _ = unsafe { RemovePropW(hwnd, w!("Corrode.TrayState")) };
			if hooked {
	...
	mod tests {
		...
		fn native_minimize_restore_restart_quit_and_cleanup() {
				...
				event_loop
					.create_window(
						winit::window::Window::default_attributes()
							.with_title("Corrode synthetic tray test")
			...
			unsafe {
				...
				drop(tray);
				assert!(IsWindowVisible(hwnd).as_bool());
				assert!(GetPropW(hwnd, w!("Corrode.TrayState")).is_invalid());
				assert_ne!(
