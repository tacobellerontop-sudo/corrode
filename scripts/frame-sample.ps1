...
.EXAMPLE
powershell -NoProfile -File scripts/frame-sample.ps1 -Executable E:\build\debug\corrode.exe -Revision abc123 -Profile debug
#>
[CmdletBinding()]
param(
...
if (-not [IO.File]::Exists($resolvedExecutable) -or [IO.Path]::GetExtension($resolvedExecutable) -ne '.exe') {
    throw 'Executable must be the exact prebuilt corrode.exe path (built with --features demo).'
}
...
try {
    ...
    while ($null -eq $summary) {
        ...
        if ($lineTask.Wait($IntervalMilliseconds)) {
            ...
            if ($line.StartsWith('{') -and $line.Contains('"corrode_frame_sample"')) {
                $record = $line | ConvertFrom-Json
                if ($record.corrode_frame_sample -eq 'start') {
                ...
                } elseif ($record.corrode_frame_sample -eq 'complete') {
