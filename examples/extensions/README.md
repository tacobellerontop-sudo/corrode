# Build your first Corrode plugin

A plugin is a function: Corrode passes it JSON, it returns JSON, and the host renders
...
python pack.py app-toolbox/manifest.json target/wasm32-unknown-unknown/release/app_toolbox.wasm packages/hello-context.corrode-extension
```
...
```powershell
cargo run --locked -p corrode -- --demo
```
...
fn missing_channel_is_handled() {
    use corrode_extension_sdk::{dispatch_typed, serde_json, AppOutput, Element};
...
```powershell
cargo doc --manifest-path examples/extensions/Cargo.toml --locked -p corrode-extension-sdk --no-deps
```
...
Authors package compiled bytes; Corrode never runs their build scripts.
...
| `corrode_alloc(i32 length) -> i32 pointer` | Allocate room for the host's UTF-8 JSON input. |
| `corrode_invoke(i32 pointer, i32 length) -> i64 output` | Return UTF-8 JSON: output pointer in the high 32 bits, byte length in the low 32 bits. |
