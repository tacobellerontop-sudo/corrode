...
//! ```
//! use corrode_extension_sdk::{dispatch, Invocation, Output, serde_json};
...
/// Export a typed handler as Corrode ABI version 1, including `fn(Invocation) -> Output`.
/// Each invocation gets a fresh instance, so buffers are reclaimed when it finishes.
/// Legacy `serein_*` aliases are exported alongside so packages built before the
/// rebrand keep validating on hosts that require them.
#[macro_export]
macro_rules! export {
	($handler:path) => {
		#[unsafe(no_mangle)]
		pub extern "C" fn corrode_alloc(length: u32) -> u32 {
		...
		/// # Safety
		/// The Corrode host supplies the pointer returned by `corrode_alloc` and its allocated length.
		#[unsafe(no_mangle)]
		pub unsafe extern "C" fn corrode_invoke(pointer: u32, length: u32) -> u64 {
		...
		pub extern "C" fn serein_alloc(length: u32) -> u32 {
			corrode_alloc(length)
		}

		/// # Safety
		/// Legacy alias for hosts built before the rebrand.
		#[unsafe(no_mangle)]
		pub unsafe extern "C" fn serein_invoke(pointer: u32, length: u32) -> u64 {
			// SAFETY: same contract as `corrode_invoke`.
			unsafe { corrode_invoke(pointer, length) }
		}
	};
}
