...
Removing a theme deletes only Corrode's copy, preserving originals and exports.
...
`corrode-clipboard-*` OS temporary directory while its file clipboard entry is
...
Disabling removes Corrode's package and extension data, and failed cleanup is
...
|---|---|---|
| Discord token | OS credential store, service `cz.viceverse.corrode`, account `discord-session` for the session restored on launch and `discord-session.<account id>` for each remembered account; at most 2048 bytes each. A per-account entry is written once, when the roster records none, and rewritten only for a token the owner just supplied | Explicit logout / Forget saved login removes both entries for that account; forgetting or pruning a saved account removes its per-account entry; invalid-token expiry also requests deletion |
...
| History and drafts | `dirs::data_local_dir()/Corrode/client.sqlite3` | Clear cached history also clears service images and keeps drafts; logout clears the authenticated account’s history and drafts |
...
| Avatar, server-icon, profile-banner and message-preview PNGs | Account subdirectory beneath `dirs::data_local_dir()/Corrode/avatars`; 1 GiB / 4096 files per account, 90 days since last use, at most 2 MiB per preview (512 KiB for icons/avatars) | Clear cache or account logout; versioned avatar/icon/banner keys and hashed media-source keys separate changed images |
...
| Authentication page | Wry incognito WebView2, destroyed on token handoff/cancel/timeout | Platform engine teardown; OS artifacts not promised erased |

Typical database directory: Windows `%LOCALAPPDATA%/Corrode`. Database contents are **not encrypted by Corrode**. OS token protection does not encrypt history, backups or drafts.
...
A process-write trace was attempted on the development host; the OS returned “a password is required.” No trace was obtained. The account/cache code and synthetic SQLite files were tested, but actual process-write behavior is not certified.
...
Voice performance diagnostics (`CORRODE_VOICE_DIAGNOSTICS=1`) are also off by default.
...
`CORRODE_MEMBER_DIAGNOSTICS=1` enables fixed-label member synchronization diagnostics;
`CORRODE_GATEWAY_DIAGNOSTICS=1` enables Gateway compatibility diagnostics. Both are off by default.
...
Output goes only to stderr. Corrode creates no log file; explicit shell redirection is owner-managed
...
overhead, separate from Corrode message/image budgets. OS font availability and
...
While enabled in an authenticated connection, Corrode owns one standard Discord IPC endpoint
and at most eight connected game workers. Windows uses a current-user-only pipe DACL and
rejects remote connections. Occupied paths are never replaced or unlinked. Windows
...
Windows uses its native tray icon adapter. The tray coalesces Show/Minimize/Quit/failure
events into a fixed atomic bitset. There is no polling/retry loop or persisted tray
history. Disabling the setting or an unavailable tray restores a hidden window;
retry requires toggling the setting.

Close keeps the process running only with an available tray. Show and Quit restore
first; Quit defers to a UI pass so existing unsaved-work and cleanup checks can run.
...
status field). Corrode reads them before gateway identify, so a launch does not
...
off by default. The sole source of truth is the `Corrode` REG_SZ value under
...
can independently block the entry; Corrode does not override that OS decision.
...
ordinary manual launches remain visible even when Start Corrode minimized is selected.
...
The updater uses a separate credential-free HTTPS client for the Corrode GitHub
