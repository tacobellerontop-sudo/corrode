...
before sending. Corrode requires Send Messages (Send Messages in Threads for threads) for
...
with an optional message ID navigate inside Corrode. Exact discord.com and legacy
...
The existing camera sender now accepts native capture on Windows (Windows Graphics Capture,
with Media Foundation and DirectShow device discovery). The adapter feeds the existing
...
[Camera in calls](voice.md#camera-in-calls-windows). Receiving video
...
before frame decryption; that changes DAVE-authenticated bytes, while Corrode's
...
SSRC, coalescing requests to at most twice per second. Windows can freshly encode
...
the chosen output resolution. Ordinary idle time does not encode extra frames. Sender
RTX repair, receiver NACK requests and adaptive bitrate remain unsupported. The viewer accepts announced RTX
...
local media handoff, not WASAPI capture, speaker output, or Discord forwarding.
Unlike Discord's selected-application mode, Corrode currently captures other
...
The standard build adds a Windows Graphics Capture sender for an existing connected DM/server voice call. Share opens a native egui source/settings dialog first. It exposes 720p/1080p and 15/30/60 fps to all accounts, plus cursor visibility; only the explicit Share screen action creates a stream. No subscription fields are changed. This paragraph describes the original video sender; subsequent audio and receive extensions supersede its original limits.

System audio extension (September 15, 2026): Windows process loopback feeds the existing
stereo Opus/DAVE stream audio sender. It is opt-in and excludes Corrode's playback, including
received call and stream audio. Other applications are included even for a single-window
share. Windows requires build 20348+ and uses native process-tree exclusion; unsupported
systems fail visibly without whole-output fallback.
...
four chunks / 153,600 bytes. The transport reserves 9,600 floats / 38,400 bytes, trims
before appending, drains a bounded queue snapshot and sends at most one 20 ms frame per
tick. Current processing buffers and native audio-server/driver storage are additional.
Rekeys and stalls clear the transport's queued/pending audio; generation tags reject
in-flight old buffers. Windows audio runs on a separate bounded worker. Synthetic checks
cover gates, bounds, sanitization, pacing and teardown; native output selection, sound
quality, echo and live interoperability remain unverified.
...
Normal-account interoperability and Windows visual equivalence remain unverified.
...
```bash
cargo run --locked -p corrode --features demo -- --demo --demo-components
```
...
Login compatibility correction (September 10): READY read_state accepts both the legacy array and the versioned entries/version/partial object, under the same 4000-entry bound. Corrode's Identify does not request the versioned_read_states capability; rejecting the legacy shape previously rejected the entire login payload. The capability's effect is described in the original [discord.py-self capability definitions](https://github.com/dolfies/discord.py-self/blob/master/discord/flags.py), rechecked September 10. Partial snapshots leave omitted channels unknown. Identify capabilities remain unchanged. Static error labels distinguish account verification, Gateway discovery, READY decoding and connection setup without exposing payloads, credentials or remote error text. Synthetic regression and loopback evidence do not establish actual account login success.
...
Corrode now enables that subscription only for the active member pane and clears it with
...
composer's ArrowUp edit shortcut. Active composition, text
...
native input-method interoperability remains unverified.
...
allows an optional guild scope. Corrode ignores guildless updates and does not request a
...
(ID only). Corrode incorrectly required a count in every group item, rejecting the complete
...
existing protocol-error behavior. Opt-in `CORRODE_GATEWAY_DIAGNOSTICS=1` now records bounded fixed
...
Games must connect to Corrode; IPC is point-to-point, not an eavesdropping/subscription feed
...
No live account/game compatibility was tested. Native execution remains unverified
...
`show_current_game` preference. Corrode now reads that preference after local opt-in
...
synthetic profile at startup for screenshots.
...
Idle, Do Not Disturb or Invisible, and apply/clear a custom status. Corrode reads
...
Decoding uses the same credential-free, validated Discord CDN
...
switches between audio and video. Clock-only UI updates run at 10 Hz; decoded
...
connects still resolves. This addresses [#237](https://github.com/ViceVerse-cz/Corrode/issues/237),
...
bounds when read back. Matching compares path suffixes longest-first; an executable-name
index keeps a full sweep at roughly 6 ms for
900 processes instead of ~920 ms for a linear scan. Processes are enumerated read-only through
`tasklist` on Windows: executable paths only, never
...
Corrode also polls the linked Spotify account at most every 15 seconds while visible (sooner
...
The offline debug command is `cargo run --locked -p corrode --features demo -- --demo --demo-check-spotify`.
...
Corrode does not download and re-upload source attachments. Basic supported message types are
