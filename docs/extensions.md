...
Corrode extensions are local, opt-in tools for the native client. The Extensions
...
[Corrode-extensions](https://github.com/ViceVerse-cz/Corrode-extensions).
...
Import selects a local `.corrode-extension` JSON package. An import is unreviewed;
importing alone does not grant it capabilities or execute it.

Disable stops accepting results immediately, then removes Corrode's downloaded
...
importing the package again and starts with fresh extension settings. Corrode
...
4. Add the package and reproducible source/build instructions to `Corrode-extensions`.
...
The in-app catalog reads `Corrode-extensions/main/catalog.json`. A new catalog entry
...
file selections must be sent or removed first. Corrode displays these attachments at
...
Plugin storage is ordinary local data, not encrypted by Corrode. Do not use it
...
bugs in the runtime or host; keep Corrode updated.

The buffer ABI is documented in the
[authoring guide](../examples/extensions/README.md#abi-version-1). The demo uses
a separate bounded temporary `corrode-extension-demo` profile; it can import local
