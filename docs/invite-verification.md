...
Choose Verify, complete the provider's check, and Corrode submits the resulting
...
same challenge shape. Corrode reuses this dialog and scopes the challenge to that single
...
that later submits the passcode (`client_core::fingerprint`).
...
Windows embeds verification in a temporary WebView2 child. The owner reported a successful
manual live check on September 13, 2026; this is not an agent-observed interoperability
test. Live verification remains unverified. Offline fixtures never
