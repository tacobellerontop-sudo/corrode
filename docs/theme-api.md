...
A `.corrode-extension` theme package contains a version 1 manifest with
...
[Corrode-extensions](https://github.com/ViceVerse-cz/Corrode-extensions), including Forest
...
restarting Corrode after enabling or disabling it. Disabled launches use an opaque
...
alpha-capable GPU surface.
The synthetic native preview can opt in without saved settings using
`cargo run --locked -p corrode --features demo -- --demo --demo-transparency`.
...
installed-theme store, while Export writes a portable `.corrode-extension` package.
