...
lengths under `Corrode.app`; compression uses `ditto -c -k --keepParent`. Executable
...
ZIPs use `ditto -c -k --keepParent` on separately preserved `Corrode.app` bundles.
...
`ditto -c -k --keepParent` on `Corrode.app` for each revision. Both bundles pass
...
baseline fails `cargo build --release --locked -p corrode --features demo` because
...
surface. An installed authenticated Corrode instance was already running and was
...
48 kHz float stereo: game `(0.125, 0.25)`, Corrode `(0.5, -0.5)`, 100 ms playback latency
...
the game-only bounds when Corrode playback appeared. Rekey, application removal,
corrode-only idle and stop also passed. This verifies the Pulse API with synthetic
...
claimed. Inspected synthetic debug framebuffer comparisons do not establish
...
debug egui/WGPU renders separately cover layout; they are not native OS
screenshots or live Discord evidence.
...
`cargo build --release --locked -p corrode --features demo` and launched with
...
Native comparison uses `cargo build --release --locked -p corrode --features demo`
...
cost of the Linux/Windows adapters. Installed size sums regular files in `Corrode.app`;
...
`cargo xtask package` could not replace the running `target/release/corrode.exe`
(Windows access denied). The newly linked `target/release/deps/corrode.exe` was
