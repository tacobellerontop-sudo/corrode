...
Voice is included in every build without a feature flag. Source builds require CMake for bundled static libopus. See [platform requirements](platform-support.md) and the [voice adapter README](../crates/discord-voice/README.md) for dependencies, exact resource limits and protocol tests.
...
The offline debug command is `cargo run --locked -p corrode --example group_call`.
...
1. Build `cargo run --locked`. Complete the [normal-user text gate](authentication.md) with the owner's Corrode session and an official Discord client. Prefer Corrode's own official-login webview; do not extract another application's credential.
...
6. Record date, OS/hardware/build features, which cases passed, redacted failures and measured resource use. Do not retain voices or private conversation contents. Record only observed behavior in the task PR description; Windows requires a physical test.
...
restarting Corrode. The watchdog cannot forcibly cancel an operating-system driver call.
...
`cargo run --locked -p corrode -- --demo --demo-voice-failed` previews a synthetic
failure and checks that subsequent cleanup/progress events retain its original reason.

Set `CORRODE_VOICE_DIAGNOSTICS=1` before launching Corrode to get aggregate voice
...
On Windows PowerShell, set `$env:CORRODE_VOICE_DIAGNOSTICS="1"` and
`$env:CORRODE_FRAME_DIAGNOSTICS="1"`, then launch `corrode.exe 2> corrode-voice.log`.
...
Corrode. Shell redirection is owner-managed and may include unrelated framework logs.
...
with no audio recordings or remote processing. Quality, CPU cost and physical latency
require owner-operated checks; this local implementation does
...
and audio remain included.

Windows uses native process loopback with `PROCESS_LOOPBACK_MODE_EXCLUDE_TARGET_PROCESS_TREE`,
excluding Corrode and its child processes across outputs. This requires Windows build 20348+
...
whole-output fallback. The capture adapter does not record to disk.

Native contract: [Microsoft process-loopback capture](https://learn.microsoft.com/en-us/samples/microsoft/windows-classic-samples/applicationloopbackaudio-sample/).

This feeds the existing stream RTC connection with 48 kHz stereo Opus at 128 kbps and
...
Windows process exclusion and actual remote sound still require owner-controlled tests;
synthetic samples do not establish those outcomes.
...
The native demo (`cargo run --locked -p corrode -- --demo --demo-voice`) exposes a synthetic picker without OS source discovery or capture. Live screen sharing requires the same owner-controlled login gate as voice testing. See [compatibility and limits](discord-compatibility.md#outgoing-screen-sharing--september-11-2026).

## Camera in calls (Windows)
...
Camera capture uses Windows Graphics Capture. Frames are 640×480, capped at 15 encoded
frames/second, encoded on a worker with a 600 kbit/s target (not a measured bandwidth
guarantee). The worker prefers the platform hardware H.264 encoder, the same Media
Foundation encoder screen sharing uses.
OpenH264 remains the fallback when no hardware encoder is available and when one fails
mid-capture, which switches the remaining capture to software rather than ending it. Every
path requests the Baseline profile and codes each picture as an IDR, so the wire format is
unchanged; a hardware encoder whose output is not independently decodable is rejected in
favor of the software one. Windows validates each native buffer against a 3,194,880-byte
ceiling (including row padding), requests one source buffer and queues at most one
921,600-byte RGB frame. One RGB preview, one encoded frame (128 KiB), and up to 256 RTP
...
Windows exposes a camera picker in Voice & Video settings and beside both call
camera controls. Discovery runs on a worker without starting capture. Up to
...
This feeds the existing camera transport; no Discord wire behavior changed in this extension.

### Screen-share audio diagnostics

The existing opt-in `CORRODE_VOICE_DIAGNOSTICS=1` reporter now also emits `StreamSend`
...
- `packets` / `rtx`: accepted video RTP packets (including restored retransmissions),
  ...
  gaps in the current picture; Corrode does not yet send NACK requests.
...
the bounded raw value is also printed as `[Corrode voice Stream] discord_delete_reason=…`.
...
`[Corrode voice Screen] capture_slow_ms=… withheld=…`. A `withheld` of zero means the desktop
...
and a later stop overwrites it: `[Corrode voice Screen] capture_stopped=…` from the capture
...
```powershell
$env:CORRODE_VOICE_DIAGNOSTICS="1"
Start-Process .\dist\corrode.exe -RedirectStandardError "$PWD\stream-debug-retest.log" -Wait
```
...
`cargo run --locked -p corrode --features demo -- --demo --demo-check-mic-preview`
